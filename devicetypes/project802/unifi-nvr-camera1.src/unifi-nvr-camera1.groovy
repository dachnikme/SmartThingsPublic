 ...
 */
metadata {
    definition (name: "UniFi NVR Camera1", namespace: "project802", author: "Chris Vincent") {
...
def take()
{
    ...
    {
        sendHubCommand( new physicalgraph.device.HubAction("""GET /api/2.0/snapshot/camera/${state.uuid}?width=480&force=true&apiKey=${key} HTTP/1.1\r\n Accept: */*\r\nHOST: ${target}\r\n\r\n""", physicalgraph.device.Protocol.LAN, "${target}", [outputMsgToS3: true, callback: nvr_cameraTakeCallback]))
    }
}
/**
...
def nvr_cameraPoll() 
{
    ...
    state.pollIsActive = true
    sendHubCommand( new physicalgraph.device.HubAction("""GET /api/2.0/camera/${state.uuid}?apiKey=${key} HTTP/1.1\r\n Accept: application/json\r\nHOST: ${target}\r\n\r\n""", physicalgraph.device.Protocol.LAN, "${target}", [callback: nvr_cameraPollCallback]))
